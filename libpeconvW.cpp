...
bool is_supported_pew(BYTE* my_exe, size_t exe_size)
{
...
bool is_supported_pe(std::string in_path) {
	//std::string in_path = msclr::interop::marshal_as<std::string>(in_path1);
	...
	bool is_ok = is_supported_pew(my_exe, exe_size);
...
namespace livpeconvW {
	int Peconv::pe_to_shc(String^ in_path)
	{
		std::string filename = msclr::interop::marshal_as<std::string>(in_path);
		//std::string in_path = filename;
		std::string  out_str = make_out_name(filename);
		if (!is_supported_pe(filename)) {
			return -2;
		}

		size_t exe_size = 0;
		BYTE* my_exe = peconv::load_pe_module(filename.c_str(), exe_size, false, false);
		if (!my_exe) {
			std::cout << "[-] Could not read the input file!" << std::endl;
			return -1;
		}
		...
		if (peconv::dump_pe(out_str.c_str(), ext_buf, ext_size, current_base, dump_mode)) {
			...
			//MessageBox(NULL, TEXT("Failed to save the output!"), TEXT("In a DLL"), MB_OK);
		}
		peconv::free_file(my_exe);
